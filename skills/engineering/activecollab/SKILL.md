...
2. Run `activecollab version --json` and require version 0.3.0 or newer. Older releases depend on a platform credential service and do not understand frontend `?modal=Task-{task_id}-{project_id}` URLs. If the version is older, stop and ask the user to upgrade the CLI before continuing.
