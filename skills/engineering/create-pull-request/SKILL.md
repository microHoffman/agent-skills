...
`gh auth status`, `gh pr create`, `gh pr view`, `gh pr edit`, `gh pr comment`,
`gh api`, and `git push` need network access and authenticated user credentials,
and sandboxed connector calls can fail with app-permission errors even when the
user's local `gh` token would work.

When using `exec_command` for these operations, set
`sandbox_permissions: "require_escalated"` with a short justification and a
scoped `prefix_rule`, for example:

- `["gh", "auth"]` for `gh auth status`
- `["gh", "pr"]` for PR create/view/edit/comment operations
- `["gh", "api"]` for REST or GraphQL calls
- `["git", "push"]` for pushing branches

Do this proactively for GitHub operations instead of trying once in the sandbox
...
2. **Detailed commit messages**: Follow conventional commit format when possible, with a concise subject and a body for non-trivial changes. When creating or amending commits, include what changed, why it changed, and relevant verification. Avoid vague one-line messages for review-driven fixes.
